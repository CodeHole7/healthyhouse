...
class ConditionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not data['custom_condition']:
            ...
            if not all([data.get('range'), data.get('type'), data.get('value')]):
                raise forms.ValidationError(
                    _("Please either choose a range, type and value OR "
                      "select a custom condition"))
...
class BenefitForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if not data['custom_benefit'] and \
                not all([data.get('range'), data.get('type'), data.get('value')]):
            raise forms.ValidationError(
                _("Please either choose a range, type and value OR "
                  "select a custom incentive"))
