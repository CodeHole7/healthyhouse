...
class Command(BaseCommand):
    """
    Overridden for adding try/except block in the bottom of `handle` method.

    Because default realisation cannot replace the `_default_manager` and
    add `rewrite` method to Manager (for all models).
    """
        ...
        for model in models:
            ...
            for field_name in opts.fields.keys():
                ...
                try:
                    ...
                    model._default_manager.filter(q).update(
                        **{def_lang_fieldname: F(field_name)})
